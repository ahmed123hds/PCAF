...
#   gcloud compute tpus tpu-vm ssh node-v4-32 --worker=all --command="cd ~/models/PCAF && bash scripts/run_tpu_v4_32_ablation.sh"

PYTHON="${PYTHON:-python}"
LOG_DIR="logs/tpu_ablation_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
