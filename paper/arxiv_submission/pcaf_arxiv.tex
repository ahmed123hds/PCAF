...
\hypersetup{
  ...
  urlcolor=blue,
  hypertexnames=false,
  pdftitle={Parallel Causal Associative Fields: Gated Sparse Memory for Long-Context Language Modeling},
  pdfauthor={Ahmed},
  pdfsubject={Parallel associative-memory language modeling}
}
...
\texttt{filliones@gmail.com}\\
\url{https://github.com/ahmed123hds/PCAF}
}
\date{June 2026}
...
single-GPU component ablations confirm that the associative cache, retrieval
...
The architectural separation at the heart of PCAF---between \emph{memory addressing} and \emph{value resolution}---is directly inspired by the complementary learning systems theory of human memory
\citep{mcclelland1995complementary,kumaran2016learning}.
In the mammalian brain, the \textbf{hippocampus} acts as a rapid-binding content-addressable index: it encodes episodic events by writing a sparse, high-dimensional pattern that links contextual cues to a stored experience.
When a familiar context recurs, the hippocampus performs \emph{pattern completion}---retrieving the associated memory from a very sparse cue, even in the presence of interference.
The \textbf{neocortex}, by contrast, slowly distills statistical regularities across many exposures into dense parametric weights, providing smooth, compositional generalization.
Crucially, these two systems operate in parallel and are gated by a learned routing signal: the brain dynamically arbitrates between direct episodic recall (``I have seen this context before, retrieve the exact successor'') and parametric generalization (``compose a novel output from learned rules'').

PCAF mirrors this two-system architecture precisely.
The \emph{parametric local path}---a causal convolutional or state-space module---plays the role of the neocortex, providing smooth, parameter-efficient modeling of local syntactic and semantic compositionality.
The \emph{content-addressed cache}---a polynomial hash-bucket memory of causal successor records---plays the role of the hippocampus, performing fast, high-fidelity retrieval of previously encountered context continuations.
The \emph{learned gate} plays the role of the hippocampal--neocortical router, dynamically weighting episodic cache recall against parametric model output at every token position.
...
The result is a model that learns \emph{when} to look up and \emph{when} to compose---a property we demonstrate empirically to be both computationally efficient and statistically powerful across long-range pretraining benchmarks.
...
\begin{enumerate}
  ...
  route addresses the ``semantic silence'' of exact token hashes by allowing
...
\caption{Overall architecture of PCAF. Input tokens are embedded and processed by causal depthwise convolutional blocks (left, teal) to produce hidden states $h_{1:T}$. An MLP head maps each queried state $h_t$ to a parametric distribution $p_{\mathrm{param}}$. In parallel, a hash address function routes causal successor records into $B$ hash buckets (right, indigo). A Triton bucket lookup retrieves at most $K$ candidates $\mathcal{C}(t)$, which are scored via key--query attention to form a cache distribution $p_{\mathrm{cache}}$. A learned sigmoid gate $\lambda_t$ mixes the two distributions to produce the final output (top, amber). In next-token diagnostics we query only the final state $h_T$; in full autoregressive training we apply the same computation at every causal position.}
...
Across all configurations, the PCAF models demonstrate substantial quality and throughput advantages.
...
On WikiText-103 at $T = 1024$, PCAF-semantic achieves a best perplexity of
\textbf{33.30} PPL (final 33.43 PPL), outperforming the dense Transformer by \textbf{16.22 points}
and the local convolutional baseline by \textbf{8.38 points}, while delivering a \textbf{2.02$\times$}
speedup in training throughput (0.89M vs 0.44M tokens/s at batch size 256).
At $T = 2048$, under a strict batch size of 32 for direct comparison, PCAF-semantic reaches a best perplexity of \textbf{36.31} PPL, outperforming the
dense Transformer baseline by \textbf{11.18 points} with a \textbf{1.42$\times$} throughput speedup (0.61M vs 0.43M tokens/s). When allowed to scale to a batch size of 256, PCAF-semantic further accelerates to \textbf{0.89M tokens/s}, representing a \textbf{2.07$\times$} hardware speedup over the best possible attention baseline.

On PG-19, the gap is even more pronounced due to the long-range dependency requirements of book-length sequences.
At $T = 1024$, PCAF-semantic achieves a best perplexity of \textbf{50.94} PPL (final 59.85 PPL), outperforming
the local convolutional baseline by \textbf{12.85 points} and the dense Transformer by \textbf{5.36 points}.
At $T = 2048$, PCAF-semantic obtains the lowest perplexity of \textbf{52.45} PPL, outperforming the local Transformer
and dense baseline while retaining a \textbf{1.44$\times$} throughput advantage at batch size 32 (0.62M vs 0.43M tokens/s), which expands to a \textbf{2.07$\times$} speedup (0.89M tokens/s) when running at batch size 256.
These results establish the central speed--quality claim: PCAF improves perplexity while moving more tokens per second than dense or local attention at the same scale.
...
PCAF substantially outperforms dense attention, sliding-window
attention, global-local attention, and local-conv ablations at both 1024- and
2048-token contexts, while achieving substantially higher training throughput
than attention-based baselines. On the larger WikiText-103 and PG-19
corpora under a full autoregressive pretraining objective on a distributed TPU v4-32 cluster,
our 303M parameter PCAF-semantic model reaches outstanding validation perplexities of
...
parallel content-addressed memory is a highly promising architectural primitive for
efficient, high-quality long-context sequence modeling at scale.
