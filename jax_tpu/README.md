...
```bash
python jax_tpu/train_pcaf_jax.py \
  ...
  --log-jsonl logs/jax_tpu_pcaf_2048.jsonl
```
...
python jax_tpu/train_pcaf_jax.py \
  ...
  --log-jsonl logs/jax_tpu_pcaf_hybrid_semantic_2048.jsonl
```
...
ROUTING_MODE=hybrid_semantic_hash SEQ_LEN=2048 GLOBAL_BATCH_SIZE=256 \
  bash scripts/run_tpu_v4_32_pcaf.sh
```
...
```bash
bash scripts/run_tpu_iclr_full_package.sh
```
