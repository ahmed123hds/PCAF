...
We trained and compared three classes of architectures on the large-scale \textbf{WikiText-103} dataset: standard dense Transformers, local and global-local attention models, and PCAF configurations (including the proposed context token-hash routing, continuous semantic-hash routing, and hybrid blends). All models are benchmarked across sequence lengths $T \in \{1024, 2048\}$. Table~\ref{tab:tpu_baselines} summarizes the baseline comparisons, Table~\ref{tab:tpu_ablations} details the PCAF routing and gating ablations, and Figure~\ref{fig:ablation_curves} displays the validation perplexity learning curves over training steps.
