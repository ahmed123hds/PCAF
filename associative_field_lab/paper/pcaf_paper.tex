...
To evaluate the scalability, efficiency, and absolute modeling strength of PCAF against standard long-context Transformer baselines, we ported our implementation to a highly optimized pure-JAX distributed codebase and conducted extensive evaluations on a Google Cloud TPU v4-32 Pod cluster (comprising 4 sharded TPU v4 nodes with 32 Tensor Cores). The JAX distributed implementation leverages SPMD sharding via \texttt{jax.experimental.shard\_map} to achieve seamless multi-worker training.
...
\begin{figure}[t]
  ...
  \caption{Validation perplexity curves over training steps on WikiText-103 at sequence lengths $T = 1024$ (left) and $T = 2048$ (right) on a TPU v4-32 cluster. The proposed \textbf{pcaf\_context} routing consistently achieves the lowest perplexity, demonstrating the power of structured associative context retrieval and learned adaptive gating over standard linear and dense attention baselines.}
  \label{fig:ablation_curves}
\end{figure}

The TPU scaling and baseline comparisons reveal three critical architectural and physical insights:
\begin{itemize}
  \item \textbf{Significant Perplexity Landslide:} The proposed \textbf{pcaf\_context} model achieves a massive modeling breakthrough compared to dense and local Transformers. At sequence length $T=2048$, \textbf{pcaf\_context} achieves a validation perplexity of \textbf{95.49}, outperforming the dense self-attention baseline (\textbf{311.40} PPL) by a factor of 3.26$\times$. This demonstrates that the non-parametric context associative cache serves as a far more effective long-range retrieval engine than standard quadratic dot-product attention in this regime.
  \item \textbf{Outstanding Computational Efficiency:} Thanks to the SPMD-sharded parallel Triton bucket lookup, integrating the associative context memory in \textbf{pcaf\_context} incurs negligible computational cost. At $T=2048$, \textbf{pcaf\_context} registers a throughput of \textbf{38.92 M tok/s}, which is **5.44$\times$ faster** than full self-attention (\textbf{7.16 M tok/s}). This highlights the subquadratic complexity advantages of our architectural choice.
  \item \textbf{Robustness of Discrete Token Hash Routing:} The discrete context token-hash routing (\textbf{pcaf\_context}) consistently outperforms learned continuous semantic routing (\textbf{pcaf\_semantic}), improving perplexity from 102.84 to 95.49 at $T=2048$. Furthermore, combining routing methods (\textbf{pcaf\_hybrid}) does not yield further benefits, indicating that raw discrete n-gram context indices provide a cleaner and more stable associative key space than continuous vector spaces.
\end{itemize}
