...
LOG_ROOT="${LOG_ROOT:-$ROOT_DIR/associative_field_lab/logs/full_ar_42m_seq2048_semantic_buckets_k32_resume_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "${JAX_PROCESS_INDEX:-0}" = "0" ]]; then
  python "$ROOT_DIR/associative_field_lab/scripts/summarize_logs.py" "$LOG_ROOT" | tee "$LOG_ROOT/summary.tsv"
fi
