...
LOG_ROOT="${LOG_ROOT:-$ROOT_DIR/associative_field_lab/logs/attn_300m_tuned_wt103_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "${JAX_PROCESS_INDEX:-0}" = "0" ]]; then
  python "$ROOT_DIR/associative_field_lab/scripts/make_paper_ready_tables.py" "$LOG_ROOT"
