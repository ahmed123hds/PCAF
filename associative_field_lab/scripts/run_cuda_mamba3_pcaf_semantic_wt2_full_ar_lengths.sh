...
LOG_ROOT="${LOG_ROOT:-associative_field_lab/logs/cuda_wt2_full_ar_mamba3_pcaf_semantic_$(date +%Y%m%d_%H%M%S)}"

mkdir -p "$LOG_ROOT"

COMMON=(
  associative_field_lab/train_lm.py
  --loss-mode full_ar
...
if [[ -f associative_field_lab/scripts/summarize_logs.py ]]; then
  "$PYTHON_BIN" associative_field_lab/scripts/summarize_logs.py "$LOG_ROOT" | tee "$LOG_ROOT/summary.tsv" || true
fi
